...
import ProductPanel from '../../components/Kids&Toys/ProductPanel';
import ListPanel from '../../components/Kids&Toys/ListPanel';
import Pagination from '../../components/CommonComponents/Pagination';
import Image from 'next/image';

export default function KidsAndToysPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Try to fetch Toys products directly from your .NET backend
        let response = await fetch('http://localhost:5192/api/products?category=Toys');
        let data = await response.json();
        
        // If no toys found, try other possible category names
        if (data.products.length === 0) {
          console.log('⚠️ No "Toys" category found, trying "Kids"...');
          response = await fetch('http://localhost:5192/api/products?category=Kids');
          data = await response.json();
        }
        
        if (data.products.length === 0) {
          console.log('⚠️ No "Kids" category found, trying client-side filtering...');
          // Fallback: Get all products and filter client-side
          response = await fetch('http://localhost:5192/api/products');
          data = await response.json();
          
          // Filter for toys/kids related products (case-insensitive)
          const kidsToysProducts = data.products.filter((product) => {
            const category = product.category.toLowerCase();
            return category.includes('toy') || 
          ...
          setProducts(kidsToysProducts);
          setFilteredProducts(kidsToysProducts);
          console.log('✅ Found', kidsToysProducts.length, 'Kids & Toys products (client-filtered)');
        } else {
          setProducts(data.products);
          setFilteredProducts(data.products);
          console.log('✅ Loaded', data.total, 'Kids & Toys products from database');
        }
        
      } catch (err) {
        console.error('❌ Error loading Kids & Toys products:', err);
        
        // Final fallback: empty state with clear message
        setProducts([]);
        setFilteredProducts([]);
      } finally {
        setLoading(false);
          ...
          <div className="flex-1 p-6">
            ...
            <div className="mb-8 relative w-full h-[210px] md:h-[280px] rounded-lg overflow-hidden">
              ...
              <div className="relative z-10 text-white p-20 text-center font-sans">
                <h1 className="text-5xl font-extrabold tracking-wide mb-4">Kids & Toys</h1>
                <p className="text-xl font-semibold">Where Tiny Hands Meet Big Adventures.</p>
              </div>
            </div>
            ...
            {currentProducts.length > 0 ? (
              ...
              <div className="text-center py-16">
                <p className="text-gray-500 text-lg">No Kids & Toys products found matching your filters.</p>
                <p className="text-gray-400 text-sm mt-2">
                  {products.length === 0 
                    ? "No toys or kids products available in the database yet." 
