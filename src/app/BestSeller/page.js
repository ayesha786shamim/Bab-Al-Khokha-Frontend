...
import ProductPanel from '../../components/BestSeller/ProductPanel';
import ListPanel from '../../components/BestSeller/ListPanel';
...
export default function BestSellerPage() {
  ...
  const [sortOption, setSortOption] = useState('Default'); // New: sort state

  const toggleMobileSidebar = () => setIsMobileSidebarOpen(!isMobileSidebarOpen);
  const closeMobileSidebar = () => setIsMobileSidebarOpen(false);

  useEffect(() => {
// NEW - Connect to your .NET backend
  const fetchData = async () => {
    try {
      // Use your real backend instead of devCollectionAPI
      const response = await fetch('http://localhost:5192/api/products');
      const data = await response.json();
      
      // Your backend returns { products: [...], total: 40 }
      setProducts(data.products);
      setFilteredProducts(data.products);
      
      console.log('✅ Loaded', data.total, 'products from backend');
    } catch (err) {
          ...
          <div className="flex-1 p-6">
            ...
            <div className="mb-8 relative w-full h-[210px] md:h-[280px] rounded-lg overflow-hidden">
              ...
              <div className="relative z-10 text-white p-20 text-center font-sans">
                <h1 className="text-5xl font-extrabold tracking-wide mb-4">BEST SELLER</h1>
