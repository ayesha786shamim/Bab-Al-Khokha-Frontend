...
import ProductPanel from '../../components/Electronics/ProductPanel';
import ListPanel from '../../components/Electronics/ListPanel';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch Electronics products directly from your .NET backend
        const response = await fetch('http://localhost:5192/api/products?category=Electronics');
        ...
        console.log('✅ Loaded', data.total, 'Electronics products from database');
      } catch (err) {
        console.error('❌ Error loading Electronics products:', err);
        ...
        try {
          ...
          const electronicsOnly = fallbackData.products.filter(
            (product) => product.category.toLowerCase() === 'electronics'
          );
          setProducts(electronicsOnly);
          setFilteredProducts(electronicsOnly);
          console.log('⚠️ Using fallback - filtered', electronicsOnly.length, 'Electronics products');
        } catch (fallbackErr) {
          ...
          <div className="flex-1 p-6">
            ...
            <div className="mb-8 relative w-full h-[210px] md:h-[280px] rounded-lg overflow-hidden">
              ...
              <div className="relative z-10 text-white p-20 text-center font-sans">
                <h1 className="text-5xl font-extrabold tracking-wide mb-4">Electronics</h1>
                <p className="text-xl font-semibold">Reliable Devices That Enhance Your Digital Experience.</p>
              </div>
            </div>
            ...
            {currentProducts.length > 0 ? (
              ...
              <div className="text-center py-16">
                <p className="text-gray-500 text-lg">No Electronics products found matching your filters.</p>
                <button
                  onClick={() => setFilters({})}
